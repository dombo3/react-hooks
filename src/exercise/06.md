...
- `Promise` resolved value can be reached through `.then(onFulfillment)`,
  rejected value from `.catch(onRejection)` or `then(..., onRejection)`
- `Promise.then().catch()` preferred over `Promise.then(..., onRejection)`
  because any error is made in `the then()` will be handled in the former

---

| return                    |     | Promise state                                           |
| :------------------------ | --- | :------------------------------------------------------ |
| returns a value           |     | Promise gets resolved with that value                   |
| returns nothing           |     | Promise gets resolved with 'undefined'                  |
| returns fulfilled Promise |     | Promise gets resolved with that Promise value           |
| \*                        |     |                                                         |
| returns rejected Promise  |     | Promise gets rejected with that Promise value           |
| throws an error           |     | Promise gets rejected with the thrown error             |
| \*                        |     |                                                         |
| returns pending Promise   |     | Promise gets resolved/rejcted depend on pending Promise |

---

- !!! `catch()` reutrns a `Promise` it calls `then()` intervally with
  `then(undefined, onRejected)` => so the same rules apply as `then()` above

---

`[rv] = await expression`

- `[rv]` Returns the fulfilled value of the promise, or the value itself if it's
  not a Promise.
- `await` Promise or any value to wait for

## Reducer reminder

`const [state, dispatch] = useReducer(reducer, initialState, initFunctionThatGetsInitialState)`

`function reducer(stateReference, dispatchArgument) { ... return newState}`
(newState will be called with setState(newState))

`function init(initialState) {...}`

`state` `dispatch(dispatchArgument) `
...
✔️ React performs Batching while updating component state for performance
improvement. Batching means grouping multiple state updates into a single
re-render.

💡 SetState is asynchronous only inside event handlers. But, synchronous inside
of async functions like Promises, setTimeout

- [Batching in React](https://crypt.codemancers.com/posts/2021-06-29-batching-in-react/)
